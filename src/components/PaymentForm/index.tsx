...
const schema = z.object({
	cardNo: z.string().min(19, { message: "Card number must be a 16-digit number" }).max(19, { message: "Card number must be a 16-digit number" }),
	expiryDate: z.string().regex(/^\d{2}\/\d{2}$/, {
		message: "Expiry date must be in the format MM/YY",
	}),
	verificationValue: z.string().min(3, { message: "CVV must be a 3-digit number" }).max(4, { message: "CVV must be a 4-digit number" }),
...
const PaymentForm = () => {
	...
	const formatCardNumber = (input: string) => {
		...
		const formattedInput = numericInput.replace(/(.*)(.{4})/, (_, prefix, lastFour) => {
				...
				>
					<div className="xs:px-5 px-2">
						<div className="mb-4 pb-3">
							...
							{errors.cardNo && <p className="text-red-500">{errors.cardNo.message}</p>}
						</div>
						<div className="flex justify-between">
							...
							<div className="mb-4 pb-3 w-2/3 ml-4">
								...
								<input
									...
									maxLength={4}
								/>
								{errors.verificationValue && <p className="text-red-500">{errors.verificationValue.message}</p>}
							</div>
						</div>
						<div className="mb-2">
